...
- [X] T013 [P] [US1] Create `backend/app/Domains/Booking/Actions/CreateBookingAction.php` accepting CreateBookingDTO, orchestrating: (1) validate tour exists and is published, (2) check idempotency key uniqueness — return existing booking if duplicate, (3) validate tour_date is future, (4) validate participant_count within group_size range, (5) call AvailabilityService to atomically check + reserve spots, (6) capture current tour pricing — if price differs from the price passed via DTO (page-load price), set a `price_changed` flag on the response so the frontend can trigger FR-027 re-confirmation step, (7) create Booking record inside DB transaction, (8) create audit log entry (action: created), (9) dispatch SendBookingConfirmationEmail job, (10) commit transaction, (11) return BookingResponseDTO
...
- [X] T020 [US1] Create `frontend/src/components/booking/BookingForm.tsx` composing ParticipantSelector, DateConfirmation, PriceBreakdown, and "Confirm Booking" button — reads tour_slug and date from URL search params, fetches tour detail for pricing, validates inputs, calls createBooking on submit, intercepts `price_changed: true` in the API response and displays a price-change confirmation modal (FR-027) requiring the traveler to explicitly re-confirm at the new price before proceeding, shows loading state, and redirects to confirmation page on success
- [X] T056 [P] [US1] Create `frontend/src/components/booking/PriceChangeModal.tsx` — modal displayed when BookingForm detects `price_changed: true`; shows old price, new price, prominent warning copy (localized via i18n), "Confirm at new price" and "Cancel" buttons; closes without booking on cancel; re-submits booking request (with `confirmed_price` param) on confirm; WCAG 2.1 AA: focus traps inside modal, Escape key closes
...
- [X] T043 [US4] Create `backend/app/Domains/Booking/Controllers/Admin/AuditController.php` with methods: `index` (query audit logs with filters, paginated) and `show` (load all audit entries for specific booking + stubbed `linked_financial_events` array until spec 008)
...
- [X] T046 [P] Create `backend/app/Domains/Booking/Jobs/SendBookingConfirmationEmail.php` as a queued job implementing ShouldQueue, dispatched after booking creation — checks `confirmation_email_sent_at` for idempotency, renders localized email template, sends via Laravel Mail, marks `confirmation_email_sent_at` timestamp on success, handles failures; on all retries exhausted, logs at ERROR severity and dispatches a `BookingEmailDeliveryFailed` event.
...
- [X] T055 [P] Add i18n completeness validation script `frontend/scripts/check-i18n-completeness.ts` that loads en.json, es.json, and it.json, computes the symmetric difference of top-level keys, and exits non-zero if any locale is missing a key present in en.json or has an extra key not in en.json. Run as part of the Phase 7 validation step (T052).
- [X] T050 [P] Create `backend/tests/Feature/Booking/ConcurrencyTest.php` covering two concurrency scenarios per SC-002: (1) simultaneous booking for the same **last single spot** — verifies exactly one succeeds and the other receives 409; (2) simultaneous booking for the same date where both travelers request **more spots than available** — verifies exactly one succeeds and availability is not over-decremented.
- [X] T051 [P] Create `backend/tests/Feature/Booking/RateLimitTest.php` verifying: (1) 429 response is returned after limit (11th request/min); (2) `Retry-After` header presence/format; (3) JSON error body contract adherence; (4) window reset; (5) response time < 100ms (SC-010).
- [X] T052 Run full flow validation per `quickstart.md`: migrate fresh, seed test data, verify all API endpoints return correct contracts, verify idempotency, verify rate limits, run all test suites (including T055 i18n completeness check)
- [X] T053 Run Lighthouse audit on booking and my-bookings pages — verify Performance score ≥ 90 per **SC-001/FR-030**. Optimize: lazy load images, code splitting for booking flow, font loading strategy inline with spec 006 T082.
- [X] T054 Run WCAG 2.1 AA accessibility audit on booking flow and my-bookings pages per **FR-031**: verify focus traps, keyboard navigation, screen reader labels, contrast, and focus indicators.
- [X] T057 [P] Create `backend/app/Domains/Booking/Jobs/AnonymizeExpiredBookingData.php` as a scheduled daily artisan command/queued job per FR-029 — queries bookings where `tour_date < NOW() - 90 days` and `anonymized_at IS NULL`, replaces traveler personal data with `ANON-{uuid}` token in a DB transaction, sets `anonymized_at` timestamp, creates a `data_anonymized` audit log entry.
- [X] T058 [P] Create `backend/app/Listeners/NotifyAdminOnEmailDeliveryFailure.php` listening to `BookingEmailDeliveryFailed` event — logs at ERROR level with context, and sends admin alert via configured notification channels. Register in `EventServiceProvider`.
