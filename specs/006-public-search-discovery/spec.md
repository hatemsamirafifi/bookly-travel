...
- What happens when a tour's price changes after a traveler loads the page but before they click "Book"? This is handled by the booking flow (spec 007), which captures the price at checkout time and informs the traveler of any mismatch. This spec (006) only displays the current price at page-load time; it does not implement price-lock or mismatch detection.
...
- What happens when a traveler searches with special characters or excessively long queries? The search query is validated at the API boundary: truncated to 255 characters, Meilisearch filter syntax operators (e.g., AND, OR, NOT, parentheses) are stripped, and slug-format parameters (category, location) are validated against `^[a-z0-9-]+$`. The frontend performs sanitization to strip script tags and renders search terms as text content (never raw HTML) to prevent XSS. See search-api.md Input Validation section for full rules.
...
- What happens when a traveler bookmarks a tour that later gets unpublished or archived? If the tour was previously published (`published_at` is non-null), the URL returns a 410 (Gone) with a message that the tour is no longer available and navigation back to search. If the tour was never published, it returns 404.
- How are tours without all translations handled? If a tour has content in English but not Spanish, the Spanish detail page falls back to English content with a `translation_warning: partial_translation` flag in the API response and a visible note to the traveler that translation is pending. Tours are never hidden due to missing translations — English fallback ensures all published tours are always discoverable.
