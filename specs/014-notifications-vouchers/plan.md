...
1. **Voucher QR payload migration** — change `VoucherService` so the QR encodes the public verification URL `https://bookly.travel/v/{booking_reference}` instead of the current JSON payload (FR-002, SC-009).
2. **Public read-only verification surface (NEW, additive)** — a thin `VerificationController` delegating to one `VerificationAction` returning a `VerificationResult` DTO serialized by a `VerificationTransformer`, plus a minimal Next.js page at `/v/{booking_reference}` (FR-021..FR-028, SC-010, SC-011). Unauthenticated, no PII, no enumeration, no side effects.
...
6. **Partner governance-email localization** — localize `PartnerApprovedMail` / `PartnerRejectedMail` / `PartnerBookingCancelledMail` / `PartnerNewBookingMail` to the partner user's `locale` with EN fallback, keeping the rejection/suspension reason (FR-006, FR-014).
...
**Performance Goals**: Confirmation email within 2 min, voucher email within 5 min of booking (SC-003); verification endpoint must be cheap (single index lookup by reference) and read-only.
...
├── resources/views/emails/partner/            # NEW per-locale views (approved/rejected/booking-cancelled/new-booking × en/es/it) with EN as the source/fallback
