...
- [x] T001 Verify existing auth events and listeners are registered in `backend/app/Providers/EventServiceProvider.php`. Confirm: `TravelerLoggedIn`, `LoginFailed`, `AccountLockedOut` events are mapped to `App\Domains\Auth\Listeners\LogAuthEvent::class`. Also: delete the stale `specs/004-traveler-signin/contracts/auth-api.md` file — it is superseded by `login-api.md` and `logout-api.md` and must be removed to avoid confusing the implementer. Additionally: verify the `throttle:auth` rate limiter is defined in `App\Providers\AppServiceProvider.php` (or `RouteServiceProvider.php`) via `RateLimiter::for('auth', ...)` and configured to 10 requests per minute per IP address. Check `->decayMinutes(1)` or equivalent decay + `->maxAttempts(10)`. If not defined, add it immediately — this is required by FR-014.
...
- [x] T008 [US1] Add login route to `backend/routes/api/public.php`. Inside the existing `Route::prefix('auth')->middleware('throttle:auth')->group(function () { ... })`, add: `Route::post('login', LoginController::class);`. Place it after the existing `Route::post('register', ...)` line. Verify in `bootstrap/app.php` or `app/Providers/AppServiceProvider.php` that the `auth` rate limiter is explicitly configured to `Limit::perMinute(10)->by($request->ip())`. Do NOT remove or modify existing routes.
...
- [x] T018 [US2] Implement or verify `frontend/src/lib/hooks/useAuth.tsx` logout integration. Read the existing `logout()` method and check ALL of the following. If any check fails, edit the file to fix it:
  ...
  3. **IMPORTANT — Provider Nesting Check**: Before adding `useLocale()` or `useRouter()`, verify in `frontend/src/app/[locale]/layout.tsx` that `AuthProvider` is rendered INSIDE `NextIntlClientProvider`, not outside it. If `AuthProvider` wraps `NextIntlClientProvider`, the `useLocale()` hook will throw a runtime error because it requires the intl context. If the nesting order is wrong, swap the providers so `NextIntlClientProvider` is the outer wrapper. Do NOT proceed to step 4 until this check passes.
  4. After clearing state, redirect to `/${locale}/` using `router.push(`/${locale}/`)`. The current `logout()` method does NOT redirect and has no access to `locale` or `router`. To fix: add `import { useLocale } from 'next-intl'` and `import { useRouter } from 'next/navigation'` to the file. Inside `AuthProvider`, call `const locale = useLocale(); const router = useRouter();`. Then add `router.push(`/${locale}/`)` as the LAST line inside the `finally` block of `logout()`, after `setIsLoading(false)` and before the closing brace.
...
- [x] T031 [P] Verify `backend/routes/api/public.php` has correct route ordering: `register`, `login`, `logout` inside the auth group with `throttle:auth` middleware. Confirm `logout` has additional `auth:sanctum` middleware.
- [x] T032 Update `frontend/src/components/auth/AuthGuard.tsx` to redirect authenticated users away from the login page to the locale-prefixed homepage.
...
- [x] T034 [P] Add performance assertion to `backend/tests/Feature/Auth/LoginTest.php`. Append test: `it('responds within 3 seconds')`: POST valid login and assert response time is under 3000ms using a `microtime(true)` wrap before/after the request and assert the delta. **Note**: This is a functional baseline check (single-request threshold). SC-002's p95 < 3s and median < 1s targets require a load testing suite (e.g., k6, Artillery) across multiple concurrent requests — that is out of scope for Phase 1 unit/integration tests.
