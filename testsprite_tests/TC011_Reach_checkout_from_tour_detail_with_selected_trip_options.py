...
async def run_test():
    ...
    try:
        ...
        # -> navigate to the seeded tour detail page directly
        ...
        # -> Click 'Book Now' to reach the checkout page.
        ...
        # Assert: Booking page URL contains '/en/booking'.
        await expect(page).to_have_url(re.compile("/en/booking"), timeout=15000)

        # --> The tour slug, participant count (2) and a concrete tour date
        #     were carried into the booking form query string. The date comes
        #     from the backend's bookable availability, so assert only that a
        #     valid future date is present instead of a hardcoded calendar day.
        url = page.url
        ...
        # --> The booking summary reflects the selected party size.
        await expect(page.get_by_text("Participants", exact=True).nth(0)).to_be_visible(timeout=15000)
        await expect(page.locator('[aria-labelledby="participants-label"]').nth(0)).to_have_text("2", timeout=15000)

        # --> The price breakdown is shown before payment is confirmed.
        await expect(page.get_by_text(re.compile("Price Breakdown", re.IGNORECASE)).first).to_be_visible(timeout=15000)
