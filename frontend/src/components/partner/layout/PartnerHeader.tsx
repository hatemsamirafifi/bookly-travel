...
export function PartnerHeader({ onMenuClick, unreadCount = 0 }: PartnerHeaderProps) {
    ...
    <header className="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 lg:px-6">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={t('notificationsLabel', { defaultValue: 'Notifications' })}
        >
          <BellIcon className="h-5 w-5" />
          {unreadCount > 0 && (
            <span className="absolute -right-0.5 -top-0.5 flex h-4.5 min-w-[1.125rem] items-center justify-center rounded-full bg-[#FFB800] px-1 text-[10px] font-bold text-[#0A2540]">
